...
东方财富妙想平台本身不提供实时持仓、交易历史的 API 数据可视化，看着数字发呆实在无聊。干脆自己动手，写了一个本地看板：历史交易、当前持仓、盈亏走势一目了然。

**未来规划：**
- 接入买卖接口，不靠 Agent 自己也能在看板里直接下单
- 盈亏历史曲线
- 多维度持仓分析
...
您的东方财富 API Key 在本地加密存储，**绝不会上传到任何服务器**。

| 安全措施 | 说明 |
|---|---|
| **本地存储** | 所有账户信息存储在本机 SQLite 数据库，不经过第三方服务器 |
| **AES-256-GCM 加密** | API Key 使用 AES-256-GCM 算法加密，需要 `ENCRYPTION_KEY` 才能解密 |
| **密钥本地生成** | `ENCRYPTION_KEY` 在首次启动时自动生成，存储在 `backend/.env` 中，**不会上传到 Git** |
| **.env 不受版本控制** | `.gitignore` 已排除 `.env`，密钥不会随代码仓库泄漏 |

### 数据流向

```
您的浏览器 → 本地后端（localhost:3001）→ 东方财富服务器
                         ↓
              仅数据库存在您本地机器
```

> **警告**：请勿将 `backend/.env` 中的 `ENCRYPTION_KEY` 透露给他人。
